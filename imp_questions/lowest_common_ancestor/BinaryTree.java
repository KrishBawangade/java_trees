package imp_questions.lowest_common_ancestor;
...
public class BinaryTree {
    ...
    public TreeNode lowestCommonAncestor(TreeNode p, TreeNode q) {
        return checkAncestor(p, q, root);   
    }

    private TreeNode checkAncestor(TreeNode p, TreeNode q, TreeNode node){
        if(node == null){
            return null;
        }
        ...
        TreeNode left = checkAncestor(p, q, node.left);
        TreeNode right = checkAncestor(p, q, node.right);
